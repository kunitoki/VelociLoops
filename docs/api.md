...
| `bit_depth` | `int32_t` | Source bit depth: 16 or 24. |
...
| `capacity` | Buffer size in frames. Must be >= `vl_get_slice_frame_count(file, index)`. |
...
`VL_ERROR_INVALID_ARG` if `left` is NULL.
